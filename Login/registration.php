...
<body>
	...
	if(isset($_POST['submit']))
	{
		...
		$prenom = htmlentities(trim($_POST['prenom']),ENT_QUOTES);
		$numtel = htmlentities(trim($_POST['numTel']),ENT_QUOTES);
		$email = htmlentities(trim($_POST['email']),ENT_QUOTES);
		$role = htmlentities(trim($_POST['Role']),ENT_QUOTES);
		$user = htmlentities(trim($_POST['user']),ENT_QUOTES);
		$pass = htmlentities(trim($_POST['pass']),ENT_QUOTES);
		...
		if($nom && $prenom && $numtel && $email && $role && $user && $pass && $confpass)
			{   
				...
				if ($longueur < 8) 
					{
						echo "Mot de passe trop court !<a href='registration.php'>ressayé</a>";
						return false;
					}
					else
					{
