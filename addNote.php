...
?>
  ...
  {
    $_POST['titreN'] = htmlentities(trim($_POST['titreN']),ENT_QUOTES);
    $_POST['contenuN'] = htmlentities(trim($_POST['contenuN']),ENT_QUOTES);
    $_POST['dateN'] = htmlentities(trim($_POST['dateN']),ENT_QUOTES);

    $titreN = $_POST['titreN'];
    $contenuN = $_POST['contenuN'];
    $dateN = $_POST['dateN'];
    $user = $_SESSION['user'];
   
    
    //htmlentities permet de viré tous les tags et trim permet de viré tous les espaces
    if($titreN && $contenuN && $user)
      {
