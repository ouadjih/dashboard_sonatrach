	...
	?>
	 <div class="form-group col  ">
	 	 <div class="col-xs-2">
		    ...
		    <input type="text" id="titren"  value="<?php echo $datas['titreN']; ?>" class="form-control">
		    <small  class="form-text text-muted">Modifier le titre de note ! </small>
	    </div>
	 	 <div class="col-xs-2">
		    <label>Contenu de note:</label>
		    <input type="text" id="contenun"  value="<?php echo $datas['ContenuN']; ?>" class="form-control">
...
<script>
  function modifier()//une fonction modifier pour modifier un article 
  {
    ...
    let titre = $("#titren").val();//#var sont les vars recuperer par id des  les inputs 
    let contenu = $("#contenun").val();
    $.ajax( //fct ajax 
    {
      type:'post',//meth post 
      data:
